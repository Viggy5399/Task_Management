...
def progr(id):
    if request.method=="GET":
        ...
        completed.insert_one({"completed":s[0]['on_progress'],"email":session['email']})
...
def update(id):
    ...
    if request.method=="POST":
        ...
        progress.update_one({"_id": ObjectId(id)},{"$set":{'on_progress':task}})
        comp_task=list(completed.find({},{"completed":1}))
        return render_template('dashboard.html',comp_v=comp_task,task = progress.find({"email":session['email']}))
...
def retrieve():
    r_task = list(retrieveall.find({"email":session["email"]},{"retrieved_tasks":1}))
    print(r_task)
...
def delete(id):
    delete_task = list(progress.find({"_id":ObjectId(id)},{"on_progress":1}))
    print(delete_task)
